...
import type { VercelRequest, VercelResponse } from 'https://esm.sh/@vercel/node';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Gmail SMTP Transporter with Debugging
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      user: user,
      pass: pass, // Should be 16-character App Password without spaces
    },
    logger: true,
    debug: true
  });
  ...
  try {
    ...
    // Clean up base64 string to get raw data
    // Handles various possible prefixes from different jsPDF versions
    const rawBase64 = pdfBase64.split(',').pop();

    const mailOptions = {
      from: `"Niamori | ნიამორი" <${user}>`,
      to: email,
      bcc: 'niamorimarketing@gmail.com', // Always send a copy to the marketing email
